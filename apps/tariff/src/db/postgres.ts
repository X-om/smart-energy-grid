/**
 * PostgreSQL Connection and Query Service
 * 
 * Manages database connection pool and provides
 * methods for tariff persistence.
 */

// Handle CommonJS pg module
import pkg from 'pg';
const { Pool } = pkg;
...
export class PostgresService {
  ...
  private async runMigrations(): Promise<void> {
    try {
      const migrationPath = join(__dirname, 'migrations', '001_create_tariffs.sql');
   ...
   */
  async insertTariff(tariff: TariffRecord): Promise<void> {
    const startTime = Date.now();

    try {
      const query = `
        INSERT INTO tariffs (
          tariff_id, region, price_per_kwh, effective_from, reason, triggered_by
        )
        VALUES ($1, $2, $3, $4, $5, $6)
      `;
   ...
   */
  async getCurrentTariff(region: string): Promise<TariffRecord | null> {
    try {
      const query = `
        SELECT 
  ...
  /**
   * Get all current tariffs (one per region)
   */
  async getAllCurrentTariffs(): Promise<Map<string, number>> {
    try {
      const query = `
   ...
   */
  async getTariffHistory(region: string, limit: number = 10): Promise<TariffRecord[]> {
    try {
      const query = `
        SELECT 
          tariff_id as "tariffId",
