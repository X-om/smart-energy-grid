...
export const createUserController = (alertManager: AlertManagerService) => {
  ...
  const getUserAlertsController = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (!meterId)
        return void res.status(400).json({ status: 'error', message: 'Missing required query parameter: meter_id' });
  ...
  const getUserAlertByIdController = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (alert.meter_id !== meterId)
        return void res.status(403).json({ status: 'error', message: 'Access denied: Alert does not belong to your meter' });
  ...
  return { getUserAlertsController, getUserAlertByIdController };
};
